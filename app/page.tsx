    ...
    <main id="top" data-font={fontPair} className={`astralla-page ${loaded ? "is-loaded" : ""}`}>
      ...
      <section className="compatibility" id="compatibility" aria-labelledby="compatibility-title">
        ...
        <div className={`compatibility-tool compatibility-tool-v2 ${compatibilityReady ? "is-result" : ""}`}>
          <div className="tool-header"><span>ЭКСПРЕСС-СОВМЕСТИМОСТЬ</span><span>РЕЗУЛЬТАТ СРАЗУ</span></div>
          {compatibilityReady ? (
            ...
            <form className="compatibility-form compatibility-form-v2" onSubmit={prepareCompatibility}>
              <div className="partner-pair">
                <label className="partner-card partner-card-self"><span className="partner-number">01</span><strong>Вы</strong><small>Ваша дата рождения</small><span className="approved-date-control"><input aria-label="Ваша дата рождения" type="text" inputMode="numeric" autoComplete="bday" placeholder="ДД.ММ.ГГГГ" pattern="[0-9]{2}[.][0-9]{2}[.][0-9]{4}" maxLength={10} value={dateOne} onChange={(event) => setDateOne(formatBirthDateInput(event.target.value))} required /><button className="approved-calendar" type="button" aria-label="Выбрать вашу дату рождения в календаре" aria-expanded={datePickerTarget === "one"} onClick={(event) => { event.preventDefault(); openDatePicker("one"); }}><i aria-hidden="true" /></button></span></label>
                <span className="partner-cross" aria-hidden="true">×</span>
                <label className="partner-card partner-card-other"><span className="partner-number">02</span><strong>Ваш партнёр</strong><small>Дата рождения партнёра</small><span className="approved-date-control"><input aria-label="Дата рождения вашего партнёра" type="text" inputMode="numeric" autoComplete="off" placeholder="ДД.ММ.ГГГГ" pattern="[0-9]{2}[.][0-9]{2}[.][0-9]{4}" maxLength={10} value={dateTwo} onChange={(event) => setDateTwo(formatBirthDateInput(event.target.value))} required /><button className="approved-calendar" type="button" aria-label="Выбрать дату рождения партнёра в календаре" aria-expanded={datePickerTarget === "two"} onClick={(event) => { event.preventDefault(); openDatePicker("two"); }}><i aria-hidden="true" /></button></span></label>
              </div>
